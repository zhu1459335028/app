        ...
        return this.each(function() {
            ...
            function init(page) {
                ...
                if (startPage>1 && startPage-1 != '1') {
                        p.push('<a class="btn btn-default ' + (option.currentPage == 1 ? 'btn-primary disabled current' : "") + '" rel="' + 1 + '">' + 1 + '</a>');
                ...
                for (var n = startPage; n <= endPage; n++) {
                    p.push('<a class="btn btn-default ' + (option.currentPage == n ? 'btn-primary disabled current' : "") + '" rel="' + n + '">' + n + '</a>');
                    //p.push(['<a class="p_btn"><span class="inner_btn"><span class="label '+className+'" onClick="_getData('+n+')" >'+n+'</span></span></a>'].join(""));
                }

                if (endPage < option.pages && option.pages - endPage != '1') {
                    p.push('<a class="dots disabled">',option.ellipseText,'</a>');
                    p.push('<a class="btn btn-default ' + (option.currentPage == option.pages ? 'btn-primary disabled current' : "") + '" rel="' + option.pages + '">' + option.pages + '</a>');
                    //p.push(['<a class="p_btn"><span class="inner_btn"><span class="label '+className+'" onClick="_getData('+n+')" >'+n+'</span></span></a>'].join(""));
